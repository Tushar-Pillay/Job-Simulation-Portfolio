# Deloitte Australia – Data Analytics Job Simulation  
**Platform: Forage**

## 📜 Certificate
![Deloitte Data Analytics Certificate](https://github.com/Tushar-Pillay/virtual-internships/blob/main/Deloitte%20Data%20Analytics/Certificate.png)

🔗 **Certificate Reference Link:**  
[View Certificate](https://www.theforage.com/completion-certificates/9PBTqmSxAf6zZTseP/io9DzWKe3PTsiS6GG_9PBTqmSxAf6zZTseP_JFEytdwinvrneF5hC_1774025695295_completion_certificate.pdf)

---

## 📘 Overview

Completed the **Deloitte Australia Data Analytics Job Simulation** on Forage,
which involved solving real-world business problems using data analysis and
forensic technology techniques.

The simulation provided hands-on experience in working with datasets,
building dashboards, and deriving actionable business insights.

---

## 🧠 Key Tasks Performed

- Completed a Deloitte job simulation involving **data analysis and forensic technology**  
- Created an **interactive data dashboard using Tableau**
      [Dashboard Tableau](https://github.com/Tushar-Pillay/virtual-internships/blob/main/Deloitte%20Data%20Analytics/Deloitte%20Manufacture%20Copmany%20task%201%20resport%20Tableau.twb)
- Used **Excel to classify data and draw business conclusions**
       [Excel Analytics](https://github.com/Tushar-Pillay/virtual-internships/blob/main/Deloitte%20Data%20Analytics/Deloitte%20task%202%20Discrimination%20Finding.xlsx)

---

## 🛠 Skills Developed

- Data Analysis  
- Data Visualization (Tableau)  
- Excel for Data Classification  
- Business Insight Generation  
- Analytical Thinking  
- Forensic Technology Basics  

---

## 🎯 Key Learnings

- Translated raw data into meaningful business insights  
- Built dashboards to support decision-making  
- Applied structured problem-solving in a consulting-style environment  
- Improved ability to communicate analytical findings effectively  

---

📌 This simulation reflects practical exposure to real-world data analytics
tasks performed in consulting and business environments.
